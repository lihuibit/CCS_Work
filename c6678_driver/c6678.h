...
#define  PLATFORM_PLL1_PLLM_val (20)//(16:800MHz)(20)//通过该参数可设置核的时钟频率(20:1.0GHz)(25:1.25GHz)
//#define  PLATFORM_PLL1_PLLM_val (20)//(20:1.0GHz)
//#define  PLATFORM_PLL1_PLLM_val (25)(25:1.25GHz)
/********************************************************************
 * DDR3部分
 *******************************************************************/
/* DDR3 PLL settings for 1333 MHz */
/* ddr3 clock=ddr3clockin*PLL PLLM value/pll PLLD value/2=(66.67Mhz*20/1/2)*2=1333MHz(demo板)*/
/*  (100MHz*10/1/2）*2= 1.0GHz)(c6678_ddr3_vpx) */
/*  (100MHz*8/1/2）*2= 800MHz)(c6678_ddr3_vpx) */
/*  (100MHz*7/1/2）*2= 700MHz)(c6678_ddr3_vpx) */
/*  (100MHz*6/1/2）*2= 600MHz)(c6678_ddr3_vpx) */
/*  (100MHz*13/1/2）*2= 1300MHz)(c6678_ddr3_vpx) */
#define PLLM_DDR3 9//19//通过配置该参数可设置DDR3跑的时钟频率
//v1.0板卡目前支持7（800MHz）和9（1000MHz），其他暂不支持，如需要请提需求
//v2.0板卡目前支持6（700MHz）和5（600MHz），其他暂不支持，如需要请提需求
//(7:输入：100MHz；输出：800MHz)（9:输入：100MHz；输出：1.0GHz）
//(12:输入：100MHz；输出：1300MHz)
//（19:输入：66.67MHz；输出：1333MHz）

#define DDR3_TYPE_1BANK_2Gb  		0  //只有一个BANK，只有正面贴了4片DDR,每片DDR为2Gb,位宽为16bit，v1.0板卡
#define DDR3_TYPE_1BANK_4Gb  		1  //只有一个BANK，只有正面贴了4片DDR,每片DDR为4Gb,位宽为16bit，v1.0板卡
#define DDR3_TYPE_2BANK_2Gb  		2	//有两个BANK，正反面均贴了DDR,共8片DDR，每片2Gb,位宽为16bit，v1.0板卡
#define DDR3_TYPE_2BANK_4Gb  		3	//有两个BANK，正反面均贴了DDR,共8片DDR，每片4Gb,位宽为16bit，v1.0板卡
#define DDR3_TYPE_2BANK_8Gb  		4	//有两个BANK，正反面均贴了DDR,共8片DDR，每片8Gb,位宽为8bit，v2.0板卡
#define DDR3_TYPE_2BANK_4Gb_1600  	5	//有两个BANK，正反面均贴了DDR,共8片新的1600的DDR芯片（旧的1333芯片已停产），每片4Gb,位宽为16bit，v1.0板卡

#define DDR3_TYPE   DDR3_TYPE_2BANK_2Gb //根据自己板子的情况来选择DDR3的类型目前只支持以上4种
...
#define DEVICE_ID1_8BIT     0x48//0xCD//0xABAB//66//
#define DEVICE_ID2_16BIT    0x4560//0x4560//
#define DEVICE_ID2_8BIT     0x44//0XF1//0xAB//0xCDCD//99//
...
#define DEVICE_ID1_8BIT     0x48//0xCD
#define DEVICE_ID2_16BIT    0xBEEF
#define DEVICE_ID2_8BIT     0x44//0xAB
 ...
 *****************************************************************************/
#define hyplnk_EXAMPLE_SERRATE_01p250 1//输出速率可改
#define hyplnk_EXAMPLE_SERRATE_03p125 2
#define hyplnk_EXAMPLE_SERRATE_06p250 3
#define hyplnk_EXAMPLE_SERRATE_07p500 4
#define hyplnk_EXAMPLE_SERRATE_10p000 5
#define hyplnk_EXAMPLE_SERRATE_12p500 6

#define	HYPLNK_RATE	hyplnk_EXAMPLE_SERRATE_07p500
...
/********************************************************************
	...
	函数名:
	  void C6678_Ddr3_Init(int Ddr3Pllm,int Ddr3Type);
	函数说明:
	  本函数是初始化ddr
	参数说明:
	  Ddr3Pllm:
...
/********************************************************************
	...
	函数名:
	  bool C6678_TimeCounter_Delaycycles(uint32_t usecs);
	函数说明:
	  本函数是delay几个周期
	参数说明:
	  usecs：周期数
...
/********************************************************************
	...
	备注:  eeprom总大小为0x10000B,使用中需注意不要超过该大小
*********************************************************************/
I2C_RET C6678_I2c_Eeprom_Read ( uint32_t byte_addr, uint32_t uiNumBytes,
                        uint8_t *puiData, uint8_t uchEepromI2cAddress);
/********************************************************************
...
/********************************************************************
	...
	参数说明:
	  p_device：设备信息结构体。
	  sector_number：擦除的sector的number(0到255)，如果if sector_number = -1, do bulk erase
	返回值:错误状态
	  SPI_EFAIL：        (SPI_STATUS)-1   错误
	  SPI_EOK  ：         0              成功
	备注: 无
...
/********************************************************************
	...
	函数名:
	  void C6678_Emif16_Fpga_Init(void)
	函数说明:
	  本函数是初始化emif16_fpga
	参数说明:
	  无
...
/********************************************************************
	...
	函数名:
	  Bool C6678_Pcie_OutboundAdrTrans(unsigned int pcieAdr)
	函数说明:
...
/********************************************************************
	...
	函数名:
		Bool C6678_Srio_Init(int Rate,int SrioId,int MultiId[3],uint8_t InfoMode);
...
/********************************************************************
	...
	函数名:
	  void  C6678_ChipInt_Clear (Uint8 ChipIntNum,Uint32 EventId);
	函数说明:
...
/********************************************************************
	...
	备注: AB mode,global region,最大长度为64MB
*********************************************************************/
...
/********************************************************************
	...
	返回值:
	  TRUE:传输成功
	  FALSE：传输失败
	备注: 支持AB，ABC模式 ,当为 ABC模式 时 （即 Srcset.Ccnt>1）nTCC=ChannelNum
*********************************************************************/
...
/********************************************************************
	...
	函数名:Bool C6678_Edma3_Sort_TransData (Uint8 ChannelCtrlNum,Uint8 ChannelNum,Int8 ShadowRegion,
	            Uint32 ParamNum,DmaSortTranParam SortTransParam,Uint8 nTCC)
	  
	函数说明:该函数是将原地址的数据经DMA传输到目的地址的同时将数据按顺时针/逆时针旋转。
	顺时针：                                                      逆时针：
    ...
    ShadowRegion: SHADOW Regions的区域号
		   ...
		   7：  CSL_EDMA3_REGION_7
    ParamNum:用于传输的第一个传输参数PARAM号
	SortTransParam：DMA sort 传输参数，具体参见结构体说明
	nTCC:nTCC=ChannelNum, edma 传输完成info,需要和ChannelNum一致
	 
	返回值:
	  TRUE:传输成功
	  FALSE：传输失败

	备注: 调用该函数调用之前需调用C6678_Edma3_Global_Init（）和C6678_Edma3_Channel_Init（）初始化
	nTCC=ChannelNum
*********************************************************************/
Bool C6678_Edma3_Sort_TransData (Uint8 ChannelCtrlNum,Uint8 ChannelNum,Int8 ShadowRegion,Uint32 ParamNum,DmaSortTranParam SortTransParam,Uint8 nTCC);
...
/********************************************************************
	...
	函数名:Bool C6678_Edma3_Pingpang_TransData_Init(DmaPingPongInitParam  PingPongInitParam,Uint8 nTCC)

	函数说明: 本函数是DMA ping pong 传输初始化函数

	参数说明: PingPongInitParam：ping pong传输的初始化参数，具体参见结构体说明
	...
	备注: AB模式
*********************************************************************/
Bool C6678_Edma3_Pingpang_TransData_Init(DmaPingPongInitParam  PingPongInitParam,Uint8 nTCC);
...
/********************************************************************
	...
	函数名:Bool C6678_Edma3_Pingpang_TransData_Signal_Triggel(Uint8 ChannelNum,Int8 ShadowRegion,Bool IntrEn,Uint8 nTCC)

	函数说明: 本函数是DMA pingpong传输的单次触发函数
	...
	备注: 该函数调用前需调用C6678_Edma3_Pingpang_TransData_Init()
*********************************************************************/
Bool C6678_Edma3_Pingpang_TransData_Signal_Triggel(Uint8 ChannelNum,Int8 ShadowRegion,Bool IntrEn,Uint8 nTCC);
...
/********************************************************************
	...
	参数说明:
	  ...
	  Counter：定时器的初始值。
	  Period：定时器的周期值。
	  Prescale:只有TimerMode==3时，这个参数才有效。pre定时器的初始值和周期值。
	  ClkinMode:输入时钟的种类。
			0：来自内部时钟。
			1：来自外部时钟。
...
/********************************************************************
	...
	函数名:
		void C6678_Timer_Start(Uint8 TimerNum,Uint8 TimerOutMode)
	函数说明:
		本函数启动计数器
	参数说明:
...
/********************************************************************
	...
	函数名:
		Bool C6678_Hypelink_Int_Hookup(Uint8 VectID,void interruptISR(),Uint8 IntNum)
...
/********************************************************************
	...
	函数名:C6678_Cache_Set(Uint8 MemType,Uint8 size)
...
/********************************************************************
	...
	函数名:Bool C6678_Flag_Multicore_Syn(Uint8 coreNum,Bool coreChoosed[])
...
/********************************************************************
	...
	函数名:Bool C6678_IPC_Intc_Clear(void)
...
/********************************************************************
	...
	函数名:
	  void C6678_Emif16_Norflash_Init(unsigned int MainRate)
	函数说明:
